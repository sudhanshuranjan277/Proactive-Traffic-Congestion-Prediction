...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Generate SUMO traffic demand for one episode."
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Random seed for departure-time jitter and connection "
            f"shuffling (default: {DEFAULT_RANDOM_SEED}). Use a "
            "different seed for each episode you want to generate, so "
            "scripts/create_dataset.py produces genuinely varied "
            "traffic data instead of the same deterministic pattern "
            "every run."
        ),
    )
